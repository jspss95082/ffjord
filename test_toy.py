...
parser.add_argument(
    '--data', choices=['swissroll', '8gaussians', 'pinwheel', 'circles', 'moons', '2spirals', 'checkerboard', 'rings', 'artifact', 'artifact_few'],
...
parser.add_argument('--data_dims', type=int, default='2')
...
if __name__ == '__main__':
    checkpoint = torch.load('artifact_768/checkpt.pth', map_location=device)
    regularization_fns, regularization_coeffs = create_regularization_fns(args)
    model = build_model_tabular(args, args.data_dims, regularization_fns).to(device)
    ...
    transformed_data = density_fn(x)
    print(transformed_data)
    print(transformed_data.shape)
    def compute_log_density(transformed_data):
        log_density = -0.5 * (transformed_data**2 + torch.log(torch.tensor(2 * math.pi)))
        return log_density

    log_density = compute_log_density(transformed_data)
    print(log_density.sum(1, keepdim=True))
    input()
